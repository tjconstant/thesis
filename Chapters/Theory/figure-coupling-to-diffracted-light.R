...
tikz("figure-coupling-to-diffracted-light.tex",width=4,height=4)
...
for(i in 1:3){
lines(x+i*2*pi/200e-9,3e8*x,col=rgb(0,0,1,1-abs(i/4)),lwd=2)
lines(-x+i*2*pi/200e-9,3e8*x,col=rgb(0,0,1,1-abs(i/4)),lwd=2)
}
points(x[x>pi/200e-9],g(x,1)[x>pi/200e-9],col=4,lwd=0.5)

#aX <- axTicks(1)
aY <- axTicks(2)

aX<-c(0,2*pi/200e-9,2*2*pi/200e-9)
#labelsX<-paste(aX/1e7,"$\\times 10^7$",sep="")
labelsX<-c("0","$k_g$","$2k_g$")
